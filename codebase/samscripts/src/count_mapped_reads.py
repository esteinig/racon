...
def CountMappedReads(sam_file, out_stats_prefix='', num_reads_in_reference_sam=1):
	...
	ret_lines += 'Number of reads in SAM file: %d (%.2f%%)\n' % (len(list(sam_reads.keys())), (float(len(list(sam_reads.keys()))) / float(num_alignments))*100.0);
	ret_lines += 'Number of mapped reads in SAM file: %d (percent_reads: %.2f%%, percent_alignments: %.2f%%)\n' % (len(list(sam_mapped_reads.keys())), (float(len(list(sam_mapped_reads.keys())) / float(len(list(sam_reads.keys()))))*100.0), (float(len(list(sam_mapped_reads.keys()))) / float(num_alignments))*100.0);
	ret_lines += 'Number of unique reads in SAM file: %d (percent_reads: %.2f%%, percent_alignments: %.2f%%)\n' % (num_unique_reads, (float(num_unique_reads) / float(len(list(sam_reads.keys()))))*100.0, (float(num_unique_reads) / float(num_alignments))*100.0);
	ret_lines += 'Number of reads with multiple alignments: %d (percent_reads: %.2f%%, percent_alignments: %.2f%%)\n' % ((len(list(sam_reads.keys())) - num_unique_reads), (float((len(list(sam_reads.keys())) - num_unique_reads)) / float(len(list(sam_reads.keys()))))*100.0, (float((len(list(sam_reads.keys())) - num_unique_reads)) / float(num_alignments))*100.0);
	ret_lines += 'Number of reads in reference SAM: %d\n' % (num_reads_in_reference_sam);
	ret_lines += 'Percent of reads compared to the reference SAM: %.2f%%\n' % ((float(len(list(sam_reads.keys()))) / float(num_reads_in_reference_sam)) * 100.0);
	ret_lines += 'Percent of mapped reads compared to the reference SAM: %.2f%%\n' % ((float(len(list(sam_mapped_reads.keys()))) / float(num_reads_in_reference_sam)) * 100.0);
	...
	ret_lines += 'Percent of reads with multiple alignments compared to the reference SAM: %.2f%%\n' % ((float(len(list(sam_reads.keys())) - num_unique_reads) / float(num_reads_in_reference_sam)) * 100.0);
	...
	ret_plot_lines += '%d\t' % (len(list(sam_reads.keys())));
	ret_plot_lines += '%d\t' % (len(list(sam_mapped_reads.keys())));
	ret_plot_lines += '%d\t' % (num_unique_reads);
	ret_plot_lines += '%d\t' % (len(list(sam_reads.keys())) - num_unique_reads);
	ret_plot_lines += '%s\t' % (num_reads_in_reference_sam);
	ret_plot_lines += '%.2f\t' % ((float(len(list(sam_reads.keys()))) / float(num_reads_in_reference_sam)) * 100.0);
	ret_plot_lines += '%.2f\t' % ((float(len(list(sam_mapped_reads.keys()))) / float(num_reads_in_reference_sam)) * 100.0);
	ret_plot_lines += '%.2f\t' % ((float(num_unique_reads) / float(num_reads_in_reference_sam)) * 100.0);
	ret_plot_lines += '%.2f\t' % ((float(len(list(sam_reads.keys())) - num_unique_reads) / float(num_reads_in_reference_sam)) * 100.0);
